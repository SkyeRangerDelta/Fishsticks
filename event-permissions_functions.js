...
async function getAccessUrl(OAuth2Client, msg) {
    ...
    msg.channel.send(`VALIDATION REQUIRED! Open this link to do so: 
${authURL}\nReply to this message with ‘!token-key [your token]’ 
to finish this process.`);
    msg.channel.send(`Event creation halted pending validation…`)
.then(sent => fishsticks.calendarAuthID = sent.id);
}
